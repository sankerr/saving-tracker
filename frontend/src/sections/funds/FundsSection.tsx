...
export default function FundsSection() {
  ...
  const holdings = useMemo(
    () => (data?.fund_holdings || []).filter((h) => !h.archived),
    [data?.fund_holdings],
  );
  ...
  async function search(value: string) {
    ...
    const j = await api<{ results?: SearchHit[] }>(
      'GET',
      `/api/funds/search?q=${encodeURIComponent(value)}&limit=15`,
      undefined,
  ...
  async function save() {
    ...
    const j = await api('POST', '/api/fund-holdings', {
      fund_id: selected.fund_id,
      data_source: selected.data_source || 'gemelnet',
      nickname,
